...
@group(2) @binding(0) var<storage, read> stars: array<StarDesc>;
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
    ...
    let instance = textureLoad(instances, coord, 0).r;
    let position = textureLoad(positions, coord, 0).xyz;
    let direction = textureLoad(directions, coord, 0).xyz;

    let star = stars[instance];

    // Compute the normal
    let normal = normalize(position - star.origin);
    ...
    let camera_position = camera.inv_view[3].xyz;

    let theta = 1.0 - dot(npos, normalize(camera_position - star.origin));

    let color = star.color + (total - 0.5) - theta;
