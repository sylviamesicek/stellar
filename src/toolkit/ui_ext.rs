...
pub trait UiExt {
    ...
    fn tokens(&self) -> &'static DesignTokens {
        super::design_tokens_of(self.theme())
    }
    ...
    fn primary_button<'a>(&mut self, atoms: impl IntoAtoms<'a>) -> egui::Response {
        self.ui_mut().add(Button::new(atoms).primary())
    }

    fn secondary_button<'a>(&mut self, atoms: impl IntoAtoms<'a>) -> egui::Response {
        self.ui_mut().add(Button::new(atoms).secondary())
    }
    ...
    //         let response = ui
    //             .add(super::icons::EXTERNAL_LINK.as_button_with_label(tokens, text))
