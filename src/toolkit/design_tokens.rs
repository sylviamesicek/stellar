...
use crate::toolkit::color_table::{ColorTable, ColorToken, Hue, Scale};
...
fn design_tokens_per_theme() -> &'static DesignTokensPerTheme {
    static DESIGN_TOKENS: OnceLock<DesignTokensPerTheme> = OnceLock::new();
    DESIGN_TOKENS
        .get_or_init(|| DesignTokensPerTheme::load().expect("Failed top load design tokens"))
}

pub fn design_tokens_of(theme: egui::Theme) -> &'static DesignTokens {
    match theme {
        egui::Theme::Dark => &design_tokens_per_theme().dark,
        egui::Theme::Light => &design_tokens_per_theme().light,
    }
}
...
impl AlertVisuals {
    fn try_get(color_table: &ColorTable, ron: &ron::Value, name: &str) -> eyre::Result<Self> {
...
impl DesignTokens {
    ...
    pub fn load(theme: Theme, tokens_ron: &str) -> eyre::Result<Self> {
        eyre::ensure!(!tokens_ron.trim().is_empty(), "Empty theme file");
...
trait RonExt {
    ...
    fn get(&self, path: &str) -> eyre::Result<&Self> {
        let mut value = self;
        for component in path.split('.') {
            if let Some(child) = value.get_child(component) {
                value = child;
            } else {
                eyre::bail!("Failed to find {component:?} in path {path:?}");
            }
        }
        Ok(value)
    }
...
fn color_from_json(color_table: &ColorTable, color_alias: &ron::Value) -> eyre::Result<Color32> {
    let color = color_alias
        .get("color")?
        .as_str()
        .ok_or_else(|| eyre::eyre!("color not a string"))?;

    let mut color = if color.starts_with('#') {
        Color32::from_hex(color)
            .map_err(|color_error| eyre::eyre!("Invalid hex color: {color_error:?}"))?
    } else if color.starts_with('{') {
        let color = color
            .strip_prefix('{')
            .ok_or_else(|| eyre::eyre!("Expected {{hue.scale}}"))?;
        let color = color
            .strip_suffix('}')
            .ok_or_else(|| eyre::eyre!("Expected {{hue.scale}}"))?;
        let (hue, scale) = color
        ...
        eyre::bail!("Expected {{hue.scale}} or #RRGGBB")
    };

    if let Ok(alpha) = color_alias.get("alpha") {
        let alpha = alpha
            .as_u8()
            .ok_or_else(|| eyre::eyre!("alpha should be an integer 0-255"))?;
...
fn try_get_scalar(json: &ron::Value, path: &str) -> eyre::Result<f32> {
    json.get(path)?
        .as_f32()
        .ok_or_else(|| eyre::anyhow!("'{path}' not a number"))
}
...
fn try_get_aliased_stroke(
    ...
    let width = color_alias
        .get("width")?
        .as_f32()
        .ok_or_else(|| eyre::anyhow!("'Alias.{alias_path}.width' not a number"))?;
