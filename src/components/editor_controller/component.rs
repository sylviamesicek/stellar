...
use glam::{DMat4, DQuat, DVec2, DVec3, Vec2, Vec3};

use crate::{
    components::Camera,
    math::{PerspectiveProjection, Projection, Transform},
};
...
impl EditorCam {
    ...
    pub fn anchor_world_space(&self, camera_transform: &Transform) -> Option<DVec3> {
    ...
    pub fn update_transform_and_projection(
        ...
        let screen_to_view_space_at_depth =
            |perspective: &PerspectiveProjection, depth: f64| -> Option<DVec2> {
                let target_size = camera.logical_size().as_dvec2();
                ...
                if !view_pos.is_finite() || view_pos3.z != depth {
                    ...
                    log::error!("Invalid view position {view_pos:?} from depth {depth}");
                    return None;
                }
                Some(view_pos)
            };

        let view_offset = match &camera.projection {
            Projection::Perspective(perspective) => {
                let Some(offset) = screen_to_view_space_at_depth(perspective, anchor.z) else {
                    log::error!("Malformed camera");
                    return;
                };
                offset
            }
        ...
        let zoom_translation_view_space = match &mut camera.projection {
            Projection::Perspective(perspective) => {
        ...
        if self.zoom_limits.zoom_through_objects
            ...
            && matches!(camera.projection, Projection::Perspective(_))
            && zoom > 0.0
        {
        ...
        transform.translation += (transform.rotation.as_dquat()
        ...
        *anchor -= pan_translation_view_space + zoom_translation_view_space;
        ...
        let anchor_world = transform.to_matrix().as_dmat4().transform_point3(*anchor);
        let orbit_dir = orbit.normalize().extend(0.0);
        let orbit_axis_world = transform
            .rotation
            .as_dquat()
        ...
        if orbit.is_finite() && orbit.length() != 0.0 {
            match self.orbit_constraint {
                OrbitConstraint::Fixed { up, can_pass_tdc } => {
                    ...
                    let angle_to_bdc = transform.forward().angle_between(up) as f64;
                    let angle_to_tdc = transform.forward().angle_between(-up) as f64;
                    let pitch_angle = {
                    ...
                    let pitch = if pitch_angle.abs() <= motion_threshold {
                        ...
                        DQuat::from_axis_angle(transform.left().as_dvec3(), pitch_angle)
                    };
                    ...
                    match [pitch == DQuat::IDENTITY, yaw == DQuat::IDENTITY] {
                        [true, true] => (),
                        [true, false] => rotate_around(transform, anchor_world, yaw),
                        [false, true] => rotate_around(transform, anchor_world, pitch),
                        [false, false] => rotate_around(transform, anchor_world, yaw * pitch),
                    };

                    let how_upright = transform.up().angle_between(up).abs();
                    // Orient the camera so up always points up (roll).
                    if how_upright > epsilon && how_upright < FRAC_PI_2 - epsilon {
                        transform.look_to(transform.forward(), up);
                ...
                OrbitConstraint::Free => {
                    ...
                    rotate_around(transform, anchor_world, rotation);
                }
            }
        }
